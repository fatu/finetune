...
class GRPOEnvConfig(TrainingArguments):
    ...
    Parameters:
        ...
        vllm_generate_mode = (`str`, *optional*, defaults to `"batch"`):
            Control the generation mode for vLLM. This setting only applies when `vllm_mode` is set to `"colocate"`.
            If you are using `vllm_mode="server"`, this parameter must be passed separately when launching the vLLM
            server via the `--vllm_generate_mode` flag. Supported values are:
            - `"batch"`: Uses the default vLLM generation mode.
            - `"one-by-one"`: Uses the fast vLLM generation mode, which is faster but may produce lower quality completions.
    ...
    vllm_generate_mode: str = field(
        default="batch",
        metadata={
            "help": "Control the generation mode for vLLM. This setting only applies when `vllm_mode` is set to "
            "`'colocate'`. If you are using `vllm_mode='server'`, this parameter must be passed separately when "
            "launching the vLLM server via the `--vllm_generate_mode` flag. Supported values are: "
            "`'batch'`: Uses the default vLLM generation mode. "
            "`'one-by-one'`: Uses the fast vLLM generation mode, which is faster but may produce lower quality "
            "completions."
        },
    )
