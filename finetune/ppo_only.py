...
from finetune.dataset_processor import SimpleGenerateCollatorWithGroundTruth
...
from finetune.vllm_utils import create_vllm_engines, init_process_group
...
class PolicyTrainerRayProcess(RayProcess):
    def from_pretrained(
        self, args: Args, model_config: ModelConfig
    ):
        self.args = args
    ...
    def train(
        ...
        # Gather weights and broadcast to vLLM engines for consistent generation
        def broadcast_to_vllm():
            # avoid OOM
            torch.cuda.empty_cache() # Free memory before large operation
            model = self.model.module   # Get unwrapped model
            count, num_params = 0, len(list(model.named_parameters())) #   Progress Tracking
            refss = []
        ...
        approxkl_stats = torch.zeros(stats_shape, device=device) # Approximate KL Divergence
        pg_clipfrac_stats = torch.zeros(stats_shape, device=device) # Policy Gradient Clip Fraction
        pg_loss_stats = torch.zeros(stats_shape, device=device) # Policy Gradient Loss
        vf_loss_stats = torch.zeros(stats_shape, device=device) # Value Function Loss
        vf_clipfrac_stats = torch.zeros(stats_shape, device=device) # Value Function Clip Fraction
        entropy_stats = torch.zeros(stats_shape, device=device) # Policy Entropy
        ratio_stats = torch.zeros(stats_shape, device=device) # Probability Ratios
        ...
        for training_step in range(resume_training_step, args.num_training_steps + 1):
            ...
            with torch.no_grad():
                ...
                responses = []
                postprocessed_responses = [] # Cleaned/decoded text versions
                logprobs = [] # Log probabilities from current policy model
                ref_logprobs = [] # Log probabilities from reference model (for KL divergence)
                scores = [] # Final reward scores for each response
                verifiable_counts = [] # Number of verifiable rewards per response
                sequence_lengths = []  # Length of each generated response
                values = [] # Value estimates from critic/value model
                per_func_rewards = {k: [] for k in reward_types} # Breakdown by reward type
                if self.rank == 0:
                ...
                # TODO: handel args.add_r1_style_format_reward
                ...
                # Mask Log Probabilities (Standard Padding)
                padding_mask = response_idxs > sequence_lengths.unsqueeze(1)
                logprobs = torch.masked_fill(logprobs, padding_mask, INVALID_LOGPROB)
                ref_logprobs = torch.masked_fill(ref_logprobs, padding_mask, INVALID_LOGPROB)
                # Mask Values (Off-by-One Pattern)
...
def main(args: Args, tc: TokenizerConfig, model_config: ModelConfig):
    ...
    inits.extend(
        model.from_pretrained.remote(args, model_config) for model in policy_group.models
    )
